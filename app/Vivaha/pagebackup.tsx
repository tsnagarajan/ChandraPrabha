// ✅ MAIN FILE — Chandra Prabha (confirmed on 2025-11-10)
// Path: ChandraPrabha/app/page.tsx
// Contains full form, logging, consent, hardened export, and 2600+ lines
...
export default function Home() {
  ...
  async function generateChart(e?: React.FormEvent) {
    ...
    try {
      ...
      try {
        await fetch(
          "https://chandraprabha-production.up.railway.app/log-chart",
          {
            method: "POST",
      ...
      console.log("Logged chart input to Railway backend");
      ...
      if (!res.ok) {
        setErr(
          json && json.error ? (json as ApiError) : { error: "Chart error" }
        );
        return;
      }
      setErr(null);
...
`}</style>
          ...
          export default function Page({ out }) {
          const printRef = useRef(null);

         // Generate summary before returning JSX
          const summary = out ? generateSummary(out) : null;
    ...
    {/* ---------- PAGE 7: Highlights from Your Chart ---------- */}
<section className="page-section print-only">
  ...
  <h2 style={{ textAlign: "center", fontSize: 20, fontWeight: 600 }}>
    Highlights from Your Chart
  </h2>

  {summary?.strengths?.length > 0 && (
    <div style={{ marginTop: 24 }}>
      <h3 style={{ fontSize: 16, fontWeight: 500 }}>Planetary Strengths</h3>
      <ul>
  ...
  {summary?.yogas?.length > 0 && (
    <div style={{ marginTop: 24 }}>
      <h3 style={{ fontSize: 16, fontWeight: 500 }}>Yogas Observed</h3>
      <ul>
        {summary.yogas.map((y, i) => (
          <li key={`yoga-${i}`}>{y}</li>
        ))}
      </ul>
    </div>
  ...
  {interpretation && (
    <div style={{ marginTop: 32 }}>
      <h3 style={{ fontSize: 16, fontWeight: 500 }}>Interpretation</h3>

      <div style={{ marginTop: 12 }}>
        <h4 style={{ fontSize: 15, fontWeight: 600 }}>Ascendant</h4>
        <p style={{ whiteSpace: "pre-line", marginTop: 6 }}>
          {interpretation.ascendantText}
        </p>
      </div>

      <div style={{ marginTop: 12 }}>
        <h4 style={{ fontSize: 15, fontWeight: 600 }}>Moon Rāsi</h4>
        <p style={{ whiteSpace: "pre-line", marginTop: 6 }}>
          {interpretation.moonText}
        </p>
      </div>

      <div style={{ marginTop: 12 }}>
        <h4 style={{ fontSize: 15, fontWeight: 600 }}>Nakshatra</h4>
        <p style={{ whiteSpace: "pre-line", marginTop: 6 }}>
          {interpretation.nakshatraText}
        </p>
      </div>

      <div style={{ marginTop: 12 }}>
        <h4 style={{ fontSize: 15, fontWeight: 600 }}>Yogas</h4>
        <p style={{ whiteSpace: "pre-line", marginTop: 6 }}>
          {interpretation.yogasText}
        </p>
      </div>

      <div style={{ marginTop: 12 }}>
        <h4 style={{ fontSize: 15, fontWeight: 600 }}>Current Dasa</h4>
        <p style={{ whiteSpace: "pre-line", marginTop: 6 }}>
          {interpretation.dasaText}
        </p>
      </div>
    </div>
  )}
  ...
  >
    These highlights are offered with respect, not as fate —
    but as a reflection of your chart’s unique rhythm.
  </p>
</section>

{/* END: printed sections */}
</div> {/* closes print-root */}
</div> {/* closes report */}
