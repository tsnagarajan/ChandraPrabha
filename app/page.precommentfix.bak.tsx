...
const TIMEZONES = [
  ...
  'America/Denver','UTC','Europe/London','Europe/Paris','Europe/Berlin',
...
function horaSign(s:number, within:number){ // D2
...
function drekkanaSign(s:number, within:number){ // D3
  const p = Math.floor(within/10); // 0..2
  const add = [0,4,8][p];
  return (s + add) % 12;
}

function saptamsaSign(s:number, within:number){ // D7
  const p = Math.floor(within/(30/7)); // 0..6
  const base = ODD(s) ? s : (s+6)%12;  // odd: from sign; even: from 7th
  return (base + p) % 12;
}

function navamsaSign(s:number, within:number){ // D9
  const p = Math.floor(within/(30/9)); // 0..8
  const q = QUALITY[s];
  const base = q==='movable' ? s : (q==='fixed' ? (s+8)%12 : (s+4)%12);
  return (base + p) % 12;
}

function dasamsaSign(s:number, within:number){ // D10
  const p = Math.floor(within/3); // 0..9
  const base = ODD(s) ? s : (s+8)%12; // odd: from sign; even: from 9th
  return (base + p) % 12;
}

function dwadasamsaSign(s:number, within:number){ // D12
  const p = Math.floor(within/2.5); // 0..11
  return (s + p) % 12;
}

function trimshamsaSign(s:number, within:number){ // D30 (Parashara)
  const odd = ODD(s);
  if (odd){
...
function vargaPlacements(positions: Record<string,number>, asc: number){
  ...
  const getDeg = (name:string)=> name==='Ascendant' ? asc : positions[name];
  return bodies.map(b=>{
    const deg = getDeg(b);
    ...
    body:string; D1:string; D2:string; D3:string; D7:string; D9:string; D10:string; D12:string; D30:string;
  }>;
}
...
useEffect(() => {
  const handler = (ev: Event) => {
    ...
    if (target.closest('#btn-download-html')) {
      ...
      try { downloadHTML(); } catch (e) { console.error(e); }
    }
  };
...
useEffect(() => {
  ...
  async function generateChart(e?: React.FormEvent) {
    ...
    try {
      ...
      setErr(null); setOut(json as ChartOut);
    } catch (e: any) { setErr({ error: e?.message || String(e) }); }
  ...
  function SouthIndianChart({ title, mode, ascDeg, positions }: {
    title: string; mode: 'sign' | 'bhava'; ascDeg?: number; positions: Record<string, number>;
  }) {
    const boxes = Array.from({length:12}).map((_,i)=>({ sign: i, signAbbr: SIGN_ABBR[i], label: '', planets: [] as string[] }));
    ...
    return (
      <div className="card avoid-break">
        ...
        <div className="charts-grid"
             style={{
               display:'grid', gridTemplateColumns:'repeat(4, var(--cell-size))', gridTemplateRows:'repeat(4, var(--cell-size))', gap:10,
               justifyContent:'center'
             }}>
          {grid.map((row, r) => row.map((cell, c) => (
            <div key={`${r}-${c}`} className="si-cell" style={{ padding:12, display:'flex', flexDirection:'column', justifyContent:'space-between', background:'#fff' }}>
              {cell ? (
                <>
                  ...
                  <div style={{display:'flex', flexWrap:'wrap', gap:8, alignItems:'center', minHeight:38}}>
                    ...
                    if (n === 0) return <span className="si-label" style={{ fontWeight: 600, opacity: .9 }}>—</span>;


                    const fontPx = n>=7 ? 10 : n>=5 ? 12 : n>=4 ? 14 : 16;
                    const pad = fontPx <= 12 ? '2px 6px' : '4px 8px';
                    return cell.planets.map((p, i) => (
                      <span key={i} className={`si-chip ${p === 'ASC' ? 'si-chip-asc' : ''}`} style={{fontSize: fontPx, padding: pad, lineHeight: 1}}>
                        {p}
                      </span>
                    ));
                  })()}
  ...
  function downloadCSV() {
    ...
    const d1Rows = d1Names.map(n => {
      ...
      if (deg === undefined) return null; return [n, fmtSignDeg(deg), fmtDMS(deg)].join(',');
    }).filter(Boolean).join('\n');
  ...
  function svgForSouthChart(title: string, mode: 'sign'|'bhava', ascDeg: number, positions: Record<string, number>) {
    ...
    if (mode === 'sign') {
      ...
      Object.entries(positions).forEach(([name, deg])=>{ const s = Math.floor(norm360(deg)/30); boxes[s].planets.push(PLANET_ABBR[name] ?? name); });
      const sAsc = Math.floor(norm360(ascDeg)/30); boxes[sAsc].planets.unshift('ASC');
    } else {
      boxes.forEach(b => { const h = ((b.sign - ascSign + 12) % 12) + 1; b.label = `H${h}`; });
      Object.entries(positions).forEach(([name, deg])=>{ const s = Math.floor(norm360(deg)/30); const house = ((s - ascSign + 12) % 12) + 1; const idx = boxes.findIndex(bb => bb.label === `H${house}`); if (idx >= 0) boxes[idx].planets.push(PLANET_ABBR[name] ?? name); });
      const idxH1 = boxes.findIndex(bb => bb.label === 'H1'); if (idxH1 >= 0) boxes[idxH1].planets.unshift('ASC');
    }
    const cell = 140, gap = 10, pad = 24;
    const width = pad*2 + gap*3 + cell*4; const height = pad*2 + gap*3 + cell*4;
    const rects: string[] = []; const labels: string[] = []; const texts: string[] = [];
    for (const {sign, row, col} of SOUTH_LAYOUT) {
      const b = boxes[sign]; const x = pad + col*(cell+gap); const y = pad + row*(cell+gap);
      rects.push(`<rect x="${x}" y="${y}" width="${cell}" height="${cell}" rx="12" ry="12" fill="#ffffff" stroke="#000000" stroke-width="3"/>`);
      labels.push(`<text x="${x+10}" y="${y+26}" font-family="system-ui,-apple-system,Segoe UI,Roboto" font-size="18" font-weight="700" fill="#000000">${b.label}</text>`);
      const line = b.planets.length ? b.planets.join(' ') : '—';
      texts.push(`<text x="${x+10}" y="${y+56}" font-family="system-ui,-apple-system,Segoe UI,Roboto" font-size="18" fill="#000000">${line}</text>`);
    }
    ...
    return `<?xml version="1.0" encoding="UTF-8"?> <svg width="${width}" height="${height+30}" viewBox="0 0 ${width} ${height+30}" xmlns="http://www.w3.org/2000/svg"> <rect x="0" y="0" width="${width}" height="${height+30}" fill="#ffffff"/> ${titleText} ${rects.join('\n')} ${labels.join('\n')} ${texts.join('\n')} </svg>`;
  }
  async function svgToPngAndDownload(svgText: string, filename: string) {
    const svgUrl = 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(svgText);
    const img = new Image(); const done = new Promise<void>((resolve, reject) => { img.onload = () => resolve(); img.onerror = (e) => reject(e); });
    img.src = svgUrl; await done; const canvas = document.createElement('canvas'); canvas.width = (img as any).width; canvas.height = (img as any).height;
    const ctx = canvas.getContext('2d')!; ctx.drawImage(img, 0, 0);
    canvas.toBlob((blob) => { if (!blob) return; const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = filename; document.body.appendChild(a); a.click(); a.remove(); setTimeout(()=>URL.revokeObjectURL(url), 1000); }, 'image/png');
  }
  async function exportChartsSVG() { if (!out) return; const d1svg = svgForSouthChart(`Rāśi (D1) — Signs${name ? ` • ${name}` : ''}`, 'sign', out.ascendant, out.positions); const d9svg = svgForSouthChart(`Navāṁśa (D9) — Signs${name ? ` • ${name}` : ''}`, 'sign', out.d9Ascendant, out.d9Positions); const bhsvg = svgForSouthChart(`Bhāva (Houses from Lagna)${name ? ` • ${name}` : ''}`, 'bhava', out.ascendant, out.positions); downloadText('chart_d1.svg', 'image/svg+xml;charset=utf-8', d1svg); downloadText('chart_d9.svg', 'image/svg+xml;charset=utf-8', d9svg); downloadText('chart_bhava.svg', 'image/svg+xml;charset=utf-8', bhsvg); }
  async function exportChartsPNG() { if (!out) return; const d1svg = svgForSouthChart(`Rāśi (D1) — Signs${name ? ` • ${name}` : ''}`, 'sign', out.ascendant, out.positions); const d9svg = svgForSouthChart(`Navāṁśa (D9) — Signs${name ? ` • ${name}` : ''}`, 'sign', out.d9Ascendant, out.d9Positions); const bhsvg = svgForSouthChart(`Bhāva (Houses from Lagna)${name ? ` • ${name}` : ''}`, 'bhava', out.ascendant, out.positions); await svgToPngAndDownload(d1svg, 'chart_d1.png'); await svgToPngAndDownload(d9svg, 'chart_d9.png'); await svgToPngAndDownload(bhsvg, 'chart_bhava.png'); }
...
function safeDownloadHTML() {
  ...
  const filename = `report${name ? '_' + name.replace(/\s+/g, '_') : ''}.html`;
   ...
   {/* Top header with centered logo (screen) */}
      <header style={{position:'sticky', top:0, background:'rgba(255,255,255,.9)', backdropFilter:'blur(6px)', borderBottom:'1px solid #eee'}}>
        <div style={{ maxWidth:1100, margin:'0 auto', padding:'16px', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', textAlign:'center' }}>
          <img src="/logo.png" alt="Chandra Prabha — Vedic Astrology" width={260} height={260} style={{display:'block', margin:'0 auto'}} onError={(e)=>{(e.target as HTMLImageElement).style.display='none';}} />
          <div style={{fontSize:14, color:'#6b7280', marginTop:6}}>Vedic Astrology Report</div>
        </div>
      </header>
    ...
    >
          ...
          <div style={{display:'grid', gridTemplateColumns:'1fr', gap:8}}>
            <label>
              <div>Name</div>
              <input value={name} onChange={e=>setName(e.target.value)} placeholder="Person's name (e.g., Durga)" style={{border:'1px solid #ddd', padding:'8px 10px', borderRadius:8, width:'100%'}} />
            </label>
          </div>
          ...
          <div style={{display:'grid', gridTemplateColumns:'1fr auto auto auto auto', gap:8}}>
            <label>
              <div>Place</div>
              <input value={place} onChange={e=>setPlace(e.target.value)} placeholder="City, State/Province, Country (e.g., Park Ridge, Illinois, USA)" style={{border:'1px solid #ddd', padding:'8px 10px', borderRadius:8, width:'100%'}} />
            </label>
            <button type="button" disabled={searching} onClick={doGeocode} style={{background:'#000', color:'#fff', border:'none', padding:'8px 12px', borderRadius:8}}>{searching ? 'Searching…' : 'Geocode'}</button>
            <button type="button" onClick={useMyLocation} style={{border:'1px solid #ddd', background:'#fff', padding:'8px 12px', borderRadius:8}}>Use my location</button>
            <button type="button" onClick={useMyTimezone} style={{border:'1px solid #ddd', background:'#fff', padding:'8px 12px', borderRadius:8}}>Use my timezone</button>
          ...
          <div style={{display:'grid', gridTemplateColumns:'repeat(5, 1fr)', gap:8}}>
            <label>
              <div>Latitude</div>
              <input type="number" step="0.000001" value={lat} onChange={e=>setLat(e.target.value===''? '' : Number(e.target.value))} placeholder="e.g., 42.011233" style={{border:'1px solid #ddd', padding:'8px 10px', borderRadius:8, width:'100%'}} />
            </label>
            <label>
              <div>Longitude</div>
...
`}</style>
            ...
            <div>
              ...
              <div style={{display:'flex', alignItems:'center', gap:6}}>
                <select value={day === '' ? '' : Number(day)} onChange={e=>setDay(e.target.value===''? '' : Number(e.target.value))} style={{border:'1px solid #ddd', padding:'8px 10px', borderRadius:8}}>
                ...
                <span>/</span>
                <select value={month === '' ? '' : Number(month)} onChange={e=>{ setMonth(e.target.value===''? '' : Number(e.target.value)); }} style={{border:'1px solid #ddd', padding:'8px 10px', borderRadius:8}}>
                  <option value="">MM</option>
                  {range(1,12).map(m => <option key={m} value={m}>{pad2(m)}</option>)}
                </select>
                <span>/</span>
                <select value={year === '' ? '' : Number(year)} onChange={e=>{ setYear(e.target.value===''? '' : Number(e.target.value)); }} style={{border:'1px solid #ddd', padding:'8px 10px', borderRadius:8}}>
            ...
            <div>
              ...
              <div style={{display:'flex', alignItems:'center', gap:6}}>
                <select value={hour12 === '' ? '' : Number(hour12)} onChange={e=>setHour12(e.target.value===''? '' : Number(e.target.value))} style={{border:'1px solid #ddd', padding:'8px 10px', borderRadius:8}}>
                  <option value="">HH</option>
                  {range(1,12).map(h => <option key={h} value={h}>{pad2(h)}</option>)}
                </select>
                <span>:</span>
                <select value={minute === '' ? '' : Number(minute)} onChange={e=>setMinute(e.target.value===''? '' : Number(e.target.value))} style={{border:'1px solid #ddd', padding:'8px 10px', borderRadius:8}}>
                  <option value="">MM</option>
                  {range(0,59).map(m => <option key={m} value={m}>{pad2(m)}</option>)}
                </select>
                <span>:</span>
                <select value={second === '' ? '' : Number(second)} onChange={e=>setSecond(e.target.value===''? '' : Number(e.target.value))} style={{border:'1px solid #ddd', padding:'8px 10px', borderRadius:8}}>
                  <option value="">SS</option>
                  {range(0,59).map(s => <option key={s} value={s}>{pad2(s)}</option>)}
                </select>
                <select value={ampm} onChange={e=>setAmpm((e.target.value || '') as 'AM'|'PM'|'')} style={{border:'1px solid #ddd', padding:'8px 10px', borderRadius:8}}>
            ...
            <label>
              <div>Timezone</div>
              <select value={tzSelect} onChange={(e) => { const v = e.target.value; setTzSelect(v); if (v !== 'CUSTOM') setTimezone(v); }} style={{border:'1px solid #ddd', padding:'8px 10px', borderRadius:8, width:'100%'}}>
                <option value="">Select timezone…</option>
                {TIMEZONES.map(tz => <option key={tz} value={tz}>{tz}</option>)}
                <option value="CUSTOM">Custom (type below)</option>
              </select>
              {tzSelect === 'CUSTOM' && (
                <input value={timezone} onChange={e=>setTimezone(e.target.value)} placeholder="e.g., America/Chicago" style={{border:'1px solid #ddd', padding:'8px 10px', borderRadius:8, width:'100%', marginTop:6}} />
              )}
            </label>
          </div>
          ...
          <div style={{display:'grid', gridTemplateColumns:'1fr auto', gap:8, alignItems:'end'}}>
            <label>
              ...
              <select value={houseSystem} onChange={e=>setHouseSystem(e.target.value)} style={{border:'1px solid #ddd', padding:'8px 10px', borderRadius:8, width:'100%'}}>
          ...
          <a href="/comments" className="text-sm text-blue-600 underline mt-1">Leave comments / feedback (optional) →</a>
          ...
          {err && (
            <div style={{color:'#b91c1c', background:'#fff', border:'1px solid #fca5a5', padding:12, borderRadius:8}}>
              <div style={{fontWeight:700, marginBottom:6}}>{typeof err === 'string' ? err : err.error}</div>
            ...
            <ul style={{display:'grid', gap:6}}>
              {filteredHits.map((h, i) => (
                <li key={i} style={{display:'flex', justifyContent:'space-between', gap:8, alignItems:'center'}}>
                  <span>{h.name}</span>
                  <button onClick={() => { setPlace(h.name); setLat(Number(h.lat.toFixed(6))); setLon(Number(h.lon.toFixed(6))); }} style={{border:'1px solid #ddd', padding:'6px 12px', borderRadius:8, background:'#fff'}}>Use</button>
                </li>
              ))}
            </ul>
          </div>
...
<div className="report-header" style={{textAlign:'center', margin:'0 0 12px'}}>
  ...
  <div style={{fontSize:18, fontWeight:900, marginTop:6}}>
    ...
    <section className="page-section">
      ...
      {/* Varga Summary (D1 → D9) */}
      <div className="card avoid-break" style={{ marginTop: 16 }}>
        <div className="section-title">Varga Summary — D1 → D9 (Navāṁśa)</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1.4fr 1.4fr 0.8fr', gap: 8 }}>
          ...
          {['Ascendant','Sun','Moon','Mercury','Venus','Mars','Jupiter','Saturn','Rahu','Ketu'].map((body) => {
            const d1 = body === 'Ascendant' ? out.ascendant : out.positions?.[body];
            const d9 = body === 'Ascendant' ? out.d9Ascendant : out.d9Positions?.[body];
            const navSize = 360 / (12 * 9); // 3°20′
            const d9Deg = typeof d9 === 'number' ? (((d9 % 360) + 360) % 360) : NaN;
            const part = Number.isFinite(d9Deg) ? (Math.floor((d9Deg % 30) / navSize) + 1) : null; // 1..9
            return (
              <React.Fragment key={`vs-${body}`}>
                <div>{body}</div>
                <div>{Number.isFinite(d1 as number) ? fmtSignDeg(d1 as number) : '—'}</div>
                <div>{Number.isFinite(d9 as number) ? fmtSignDeg(d9 as number) : '—'}</div>
                <div>{part ?? '—'}</div>
              </React.Fragment>
            );
          })}
        ...
        <div style={{ fontSize: 12, color: '#6b7280', marginTop: 6 }}>
          “Navāṁśa part” is the 1–9 sub-division within the D9 sign (each is 3°20′).
        </div>
      </div>
    </section>
...
<section className="page-section">
  <div className="card avoid-break">
    ...
    <div style={{display:'grid', gridTemplateColumns:'1.3fr repeat(8, 1fr)', gap:8}}>
      <div style={{fontWeight:900}}>Body</div>
      ...
      {vargaPlacements(out.positions, out.ascendant).map((row, i)=>(
    ...
    <div style={{ fontSize: 12, color: '#6b7280', marginTop: 6 }}>
      Signs only are computed from D1 longitudes using standard varga rules (no degrees shown).
    </div>
  </div>
</section>
    ...
    <section className="page-section">
      ...
      <div className="card avoid-break">
        ...
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:8}}>
          ...
          {mergeAspects(out.aspects || [], deriveAscAspects(out)).length === 0 && (
