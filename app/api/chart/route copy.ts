...
import { calculateBhavaBala } from "@/app/lib/bhavabala-logic";
...
const declinations: Record<string, number> = {};   // ← ADD THIS
const serrMap: Record<string, string> = {};


  for (const [name, code] of plist) {
    const res = await sweCalcUT(jd_ut, code, iflag | swe.FLG_SPEED);
    ...
    declinations[name] = Array.isArray(res.xx) ? res.xx[3] : 0;

    
    // Capture the speed (daily motion in degrees)
    // res.xx[3] is the standard index for speed in swisseph
    // Speed (daily motion, degrees/day)
const spRaw =
  (Array.isArray(res?.xx) && typeof res.xx[3] === "number") ? res.xx[3] :
  (typeof (res as any)?.speed === "number") ? (res as any).speed :
  (typeof (res as any)?.speedLong === "number") ? (res as any).speedLong :
  (typeof (res as any)?.lonSpeed === "number") ? (res as any).lonSpeed :
  (typeof (res as any)?.longitudeSpeed === "number") ? (res as any).longitudeSpeed :
  NaN;

const sp = Number.isFinite(spRaw) ? spRaw : 0;

// Keep your existing 2-letter keys (for old Vakra/UI code)
const ab = name.slice(0, 2); // "Su", "Mo", "Me"...
speeds[ab] = sp;

// ALSO store under keys Shadbala expects
...
// And also by full planet name (optional but handy)
speeds[name] = sp;
...
// Keep speeds for display, but do NOT allow Rahu/Ketu to be treated as Vakra
...
speeds["KETU"] = Math.abs(speeds["KETU"] ?? 0);
  ...
  return { ok: true as const, positions, speeds, serrMap };
}
...
function buildNakTable(jd_ut: number, positions: Record<string, number>) {
  const nakTable: any[] = [];

  for (const planet in positions) {
    const lon = positions[planet];
    const nak = Math.floor((lon % 360) / (13 + 1/3)) + 1;
...
function buildVimDasha(jd_ut: number, positions: Record<string, number>) {
  const sunLon = positions["Sun"];
  if (sunLon === undefined) {
    throw new Error("Sun longitude missing for Vimshottari Dasha");
  }

  // Nakshatra index (0–26)
  const nakIndex = Math.floor((sunLon % 360) / (13 + 1/3));
  ...
  const lord = seq[nakIndex];
  const years = dashaYears[lord];

  return {
    startNakshatra: nakIndex + 1,
    lord,
    years
  };
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!dtUTC.isValid) {
     ...
     // --- CONSENT LOGGING (optional; never blocks chart response) ---
try {
  // Accept consent from either the API URL OR the page URL (Referer)
  const urlConsent = new URL(req.url).searchParams.get("consent") === "1";
  ...
  if (consent && process.env.SHEETS_WEBHOOK_URL) {
    ...
    const payloadForSheet = {
      name: body?.name ?? "",        // if your form doesn't send these, empty is fine
      date: String(date ?? ""),
      time: String(time ?? ""),
      timezone: String(timezone ?? ""),
      place: body?.place ?? "",      // optional
      lat: String(lat ?? ""),
      lon: String(lon ?? "")
    };

    // Fire-and-forget so chart generation isn’t delayed
  ...
  { name: 'SWIEPH', iflag: IF_SWIEPH }
    ...
    for (const eng of tryOrder) {
      ...
      if (attempt.ok) {
       ...
       speeds = attempt.speeds;
       declinations = attempt.declinations;   // ← ADD THIS LINE
       lastError = null;
       break;
}
...
let cusps: number[] = [];
let ascendant = 0;
let hRaw: any = null;   // ⭐ FIX: declare hRaw OUTSIDE the try

try {
  hRaw = swe.swe_houses(jd_ut, lat, lon, houseSystem);

  if (!hRaw) {
    throw new Error("swe_houses returned null");
  }

  if (!hRaw) {
  throw new Error("No house data returned");
}
...
// --- SHADBALA + BHAVA BALA INPUT ---
const shadInput = {
  planets: Object.fromEntries(
    Object.entries(positions).map(([planet, lon]) => [
      planet,
      {
        lon,
        speed: speeds[planet]
  ...
  ascLongitude: ascendant.degree,
  isDayBirth: dtLocal.hour >= 6 && dtLocal.hour < 18,
  weekday: dtLocal.weekday,
  sunrise: sunriseISO,
  sunset: sunsetISO
};
...
// --- COMPUTE SHADBALA ---
const shadbala = computeShadbala(shadInput);

// Extract totals for Adhipati Bala
const shadbalaTotals: Record<string, number> = {};
for (const planet of Object.keys(shadbala)) {
  shadbalaTotals[planet] = shadbala[planet].total;
}

// Build planetPositions[] for Bhava Bala
const planetPositions = Object.keys(positions).map(name => ({
  name,
  degree: positions[name],
  isBenefic: true,
  isMalefic: false
}));


const houseLords = getHouseLords(ascendant.sign);


// Build Bhava Bala input
const bhavaInput = {
  houseCusps: cusps,
  planetPositions,
...
// --- COMPUTE BHAVA BALA ---
const bhavabala = computeBhavabala(bhavaInput);
